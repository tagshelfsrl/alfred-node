...
import { DownloadResult, FileUploadPayload, UploadFilePayload } from "../../interfaces";
...
async download(id: string): Promise<DownloadResult> {
  ...
  async uploadFile(payload: UploadFilePayload): Promise<{ fileId: string; }> {
    ...
    const resp = await this.client._http.post<{ fileId: string }>('api/file/uploadfile', formData, {
  ...
  async upload(payload: FileUploadPayload): Promise<{ fileId: string; }> {
    const resp = await this.client._http.post<{ fileId: string }>('api/file/upload', payload);
